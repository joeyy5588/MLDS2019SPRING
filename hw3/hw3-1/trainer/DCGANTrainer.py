    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, real_images  in enumerate(self.dataloader):
            ...
            print('[%d/%d] Loss_D: %.4f Loss_G: %.4f D(x): %.4f D(G(z)): %.4f / %.4f'%\
             (batch_idx, len(self.dataloader), loss_d.item(), loss_g.item(), D_x, D_G_z1, D_G_z2))
        ...
        print("======================================================================================")
        print('FINISH EPOCH: [%d/%d] Loss_D: %.4f Loss_G: %.4f'% (epoch, self.n_epochs, D_sum_loss, G_sum_loss))
        print("======================================================================================")
        if epoch%5 == 0:
            with torch.no_grad():
                fixed_image = self.gen(self.fixed_noise)
                save_image(fixed_image.data[:25], "saved/images_%d.png" % epoch, nrow = 5, normalize = True)
