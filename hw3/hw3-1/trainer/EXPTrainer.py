...
class EXPTrainer:    
    def __init__(self, gen, dis, dataloader, opt):
        ...
        self.gen_iter = 1
        self.dis_iter = 1
        self.gen_optimizer = torch.optim.Adam(self.gen.parameters(), lr = 0.0002, betas=(0.5, 0.999)) # torch.optim.SGD(self.gen.parameters(), lr = 0.01)
        self.dis_optimizer = torch.optim.Adam(self.dis.parameters(), lr = 0.0002, betas=(0.5, 0.999)) # torch.optim.SGD(self.dis.parameters(), lr = 0.001)
    ...
    def _train_epoch(self, epoch):
        ...
        self.logger.info("======================================================================================")
        self.logger.info('FINISH EPOCH: [%d/%d] Loss_D: %.4f Loss_G: %.4f'% (epoch + 1, self.n_epochs, D_sum_loss, G_sum_loss))
        self.logger.info("======================================================================================")
        if (epoch + 1)%1 == 0:
            with torch.no_grad():
