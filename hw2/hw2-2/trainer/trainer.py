...
class Trainer:    
    def __init__(self, model, lang, dataloader, val_dataloader, opt):
    ...
    def _train_epoch(self, epoch):
        ...
        for i, ((q_idxs, a_idxs)) in enumerate(dataloader):
            ...
            out = model(q_idxs, a_idxs)#, p)[:len(sens), :, :]
            ...
            if i % 100 == 0:
                self._log_random_sentences(total_sen, num = 5)
        log = {
            'epoch': epoch,
    ...
    def _val_epoch(self, epoch):
        ...
        for i, ((q_idxs, a_idxs), sens) in enumerate(dataloader):
            q_idxs, a_idxs = q_idxs.to(device), a_idxs.to(device)
            out = model(q_idxs, a_idxs)[:len(sens), :, :]
            total_sen['gt'] = torch.cat((total_sen['gt'], a_idxs), dim = 0)
            total_sen['pred'] = torch.cat((total_sen['pred'], torch.argmax(out, dim = 2)), dim = 0)
